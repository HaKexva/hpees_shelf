...
# Recreates the `demo_*` prefixed table set **alongside** normal app tables (`users`,
# `books`, …). Migrations run on both `primary` and `primary_shard_demo`, so these
# names exist in `public` on the main DB and in schema `demo` on the demo shard.
class CreateDemoTable < ActiveRecord::Migration[8.1]
  def up
